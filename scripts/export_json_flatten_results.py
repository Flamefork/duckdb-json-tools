#!/usr/bin/env python3
"""Aggregate json_flatten benchmark results and refresh the summary documentation."""
...
DEFAULT_SUMMARY_CSV = PROJECT_ROOT / "bench" / "results" / "json_flatten_summary.csv"
DEFAULT_DOC_PATH = PROJECT_ROOT / "docs" / "benchmarks" / "json_flatten.md"
...
paired AS (
    SELECT
        j.scenario,
        j.thread_count,
        j.avg_wall_ms AS json_flatten_ms,
        j.std_wall_ms AS json_flatten_std_ms,
        j.median_wall_ms AS json_flatten_median_ms,
        j.avg_rps AS json_flatten_rps,
        j.std_rps AS json_flatten_std_rps,
        j.median_rps AS json_flatten_median_rps,
        j.run_count AS json_flatten_runs,
    ...
    WHERE j.implementation = 'json_flatten'
      AND m.implementation = 'flatten_json'
)
SELECT
    scenario,
    thread_count,
    json_flatten_ms,
    macro_ms,
    macro_ms / NULLIF(json_flatten_ms, 0) AS speedup,
    json_flatten_std_ms,
    macro_std_ms,
    json_flatten_median_ms,
    macro_median_ms,
    json_flatten_rps / 1000.0 AS json_flatten_krows_s,
    macro_rps / 1000.0 AS macro_krows_s,
    json_flatten_std_rps / 1000.0 AS json_flatten_std_krows_s,
    macro_std_rps / 1000.0 AS macro_std_krows_s,
    json_flatten_median_rps / 1000.0 AS json_flatten_median_krows_s,
...
def build_markdown_table(rows: Sequence[Dict[str, object]]) -> str:
    ...
    header = [
        ...
        "flatten_json (ms)",
        "Speedup",
        "json_flatten (krows/s)",
        "flatten_json (krows/s)",
    ]
    lines: List[str] = []
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    format_float(row["json_flatten_ms"]),
                    format_float(row["macro_ms"]),
                    format_float(row["speedup"]),
                    format_float(row["json_flatten_krows_s"]),
