#!/usr/bin/env python3
"""Compare two json_flatten benchmark summary CSVs and report deltas."""
...
class SummaryRow:
    ...
    flatten_json_ms: float
    speedup: float
    json_flatten_std_ms: float
    flatten_json_std_ms: float
    json_flatten_median_ms: float
    flatten_json_median_ms: float
    json_flatten_krows_s: float
    flatten_json_krows_s: float
    json_flatten_std_krows_s: float
    flatten_json_std_krows_s: float
    json_flatten_median_krows_s: float
    flatten_json_median_krows_s: float
    json_flatten_runs: int
    flatten_json_runs: int
    ...
    with path.open("r", encoding="utf-8") as handle:
        ...
        for raw in reader:
            ...
            rows[key] = SummaryRow(
                ...
                json_flatten_ms=parse_float(raw["json_flatten_ms"]),
                flatten_json_ms=parse_float(raw["macro_ms"]),
                speedup=parse_float(raw["speedup"]),
                json_flatten_std_ms=parse_float(raw["json_flatten_std_ms"]),
                flatten_json_std_ms=parse_float(raw["macro_std_ms"]),
                json_flatten_median_ms=parse_float(raw["json_flatten_median_ms"]),
                flatten_json_median_ms=parse_float(raw["macro_median_ms"]),
                json_flatten_krows_s=parse_float(raw["json_flatten_krows_s"]),
                flatten_json_krows_s=parse_float(raw["macro_krows_s"]),
                json_flatten_std_krows_s=parse_float(raw["json_flatten_std_krows_s"]),
                flatten_json_std_krows_s=parse_float(raw["macro_std_krows_s"]),
                json_flatten_median_krows_s=parse_float(raw["json_flatten_median_krows_s"]),
                flatten_json_median_krows_s=parse_float(raw["macro_median_krows_s"]),
                json_flatten_runs=int(raw["json_flatten_runs"] or 0),
                flatten_json_runs=int(raw["macro_runs"] or 0),
            )
    return rows
    ...
    for key in sorted(set(baseline.keys()) | set(candidate.keys())):
        ...
        if not base_row or not cand_row:
            ...
            rows.append([
                scenario,
                str(thread),
                format_float(base_row.json_flatten_ms if base_row else None),
                format_float(cand_row.json_flatten_ms if cand_row else None),
                "",
                status,
                format_float(base_row.speedup if base_row else None),
                format_float(cand_row.speedup if cand_row else None),
                "",
                format_float(base_row.json_flatten_krows_s if base_row else None),
                format_float(cand_row.json_flatten_krows_s if cand_row else None),
                "",
            ])
            continue
        delta_ms = cand_row.json_flatten_ms - base_row.json_flatten_ms
        delta_pct = percent_change(cand_row.json_flatten_ms, base_row.json_flatten_ms)
        delta_speedup = cand_row.speedup - base_row.speedup
        delta_krows = cand_row.json_flatten_krows_s - base_row.json_flatten_krows_s
        base_sigma = base_row.json_flatten_std_ms
        ...
        row = [
            ...
            format_float(base_row.json_flatten_ms),
            format_float(cand_row.json_flatten_ms),
            ...
            format_float(base_row.json_flatten_krows_s),
            format_float(cand_row.json_flatten_krows_s),
...
REQUIRED_FIELDS = {
    ...
    "json_flatten_std_ms",
    "macro_std_ms",
    "json_flatten_median_ms",
    "macro_median_ms",
    "json_flatten_krows_s",
    "macro_krows_s",
    "json_flatten_std_krows_s",
    "macro_std_krows_s",
    "json_flatten_median_krows_s",
