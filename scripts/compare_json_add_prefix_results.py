#!/usr/bin/env python3
"""Compare two json_add_prefix benchmark summary CSVs and report deltas."""
...
class SummaryRow:
    scenario: str
    thread_count: int
    json_add_prefix_ms: float
    add_prefix_json_ms: float
    speedup: float
    json_add_prefix_std_ms: float
    add_prefix_json_std_ms: float
    json_add_prefix_median_ms: float
    add_prefix_json_median_ms: float
    json_add_prefix_krows_s: float
    add_prefix_json_krows_s: float
    json_add_prefix_std_krows_s: float
    add_prefix_json_std_krows_s: float
    json_add_prefix_median_krows_s: float
    add_prefix_json_median_krows_s: float
    json_add_prefix_runs: int
    add_prefix_json_runs: int
    ...
    with path.open("r", encoding="utf-8") as handle:
        ...
        for raw in reader:
            ...
            rows[key] = SummaryRow(
                ...
                json_add_prefix_ms=parse_float(raw["json_add_prefix_ms"]),
                add_prefix_json_ms=parse_float(raw["macro_ms"]),
                speedup=parse_float(raw["speedup"]),
                json_add_prefix_std_ms=parse_float(raw["json_add_prefix_std_ms"]),
                add_prefix_json_std_ms=parse_float(raw["macro_std_ms"]),
                json_add_prefix_median_ms=parse_float(raw["json_add_prefix_median_ms"]),
                add_prefix_json_median_ms=parse_float(raw["macro_median_ms"]),
                json_add_prefix_krows_s=parse_float(raw["json_add_prefix_krows_s"]),
                add_prefix_json_krows_s=parse_float(raw["macro_krows_s"]),
                json_add_prefix_std_krows_s=parse_float(raw["json_add_prefix_std_krows_s"]),
                add_prefix_json_std_krows_s=parse_float(raw["macro_std_krows_s"]),
                json_add_prefix_median_krows_s=parse_float(raw["json_add_prefix_median_krows_s"]),
                add_prefix_json_median_krows_s=parse_float(raw["macro_median_krows_s"]),
                json_add_prefix_runs=int(raw["json_add_prefix_runs"] or 0),
                add_prefix_json_runs=int(raw["macro_runs"] or 0),
            )
    return rows
    ...
    for key in sorted(set(baseline.keys()) | set(candidate.keys())):
        ...
        if not base_row or not cand_row:
            ...
            rows.append([
                scenario,
                str(thread),
                format_float(base_row.json_add_prefix_ms if base_row else None),
                format_float(cand_row.json_add_prefix_ms if cand_row else None),
                "",
                status,
                format_float(base_row.speedup if base_row else None),
                format_float(cand_row.speedup if cand_row else None),
                "",
                format_float(base_row.json_add_prefix_krows_s if base_row else None),
                format_float(cand_row.json_add_prefix_krows_s if cand_row else None),
                "",
            ])
            continue
        delta_ms = cand_row.json_add_prefix_ms - base_row.json_add_prefix_ms
        delta_pct = percent_change(cand_row.json_add_prefix_ms, base_row.json_add_prefix_ms)
        delta_speedup = cand_row.speedup - base_row.speedup
        delta_krows = cand_row.json_add_prefix_krows_s - base_row.json_add_prefix_krows_s
        base_sigma = base_row.json_add_prefix_std_ms
        ...
        row = [
            ...
            format_float(base_row.json_add_prefix_ms),
            format_float(cand_row.json_add_prefix_ms),
            ...
            format_float(base_row.json_add_prefix_krows_s),
            format_float(cand_row.json_add_prefix_krows_s),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--baseline", type=Path, required=True, help="Baseline summary CSV")
    parser.add_argument("--candidate", type=Path, required=True, help="Candidate summary CSV to compare")
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional path to write a Markdown table instead of printing to stdout",
    )
    parser.add_argument(
        "--tolerance-pct",
        type=float,
        default=2.0,
        help="Ignore wall-time deltas within this percent threshold (default: %(default)s)",
    )
    parser.add_argument(
        "--sigma-threshold",
        type=float,
        default=2.0,
        help="Require latency deltas to exceed this multiple of the baseline standard deviation",
    )
...
REQUIRED_FIELDS = {
    ...
    "macro_median_ms",
    "json_add_prefix_krows_s",
    "macro_krows_s",
    "json_add_prefix_std_krows_s",
    "macro_std_krows_s",
    "json_add_prefix_median_krows_s",
    "macro_median_krows_s",
